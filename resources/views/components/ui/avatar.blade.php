...
<div class="relative inline-flex flex-shrink-0 self-start @if($size === 'full') w-full h-full @endif">
    ...
    >
        @if($src)
            ...
            />
            <svg viewBox="0 0 100 100" class="w-full h-full text-current flex-shrink-0 hidden" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        @else
            <svg viewBox="0 0 100 100" class="w-full h-full text-current flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <rect width="100" height="100" fill="{{ $palette['bg'] }}"/>
                <!-- Head (rounder vertical oval) -->
                <ellipse cx="50" cy="38" rx="19" ry="21" fill="{{ $palette['fg'] }}"/>
                <!-- Body (curved shoulders reaching bottom) -->
