...
- **Azure VM is the primary production host** — GitHub Actions deploys to the VM over SSH, builds a Docker image on the server, then restarts the `ueconnect` container.
- **Host Nginx owns public traffic** — VM Nginx terminates `80/443` for `ueconnect.io.vn` and proxies to Docker on `127.0.0.1:10000`.
- **Reverb behind in-container Nginx** — the Docker container runs Nginx, PHP-FPM, Reverb on `8080`, and a queue worker under Supervisord.
...
```env
# Low-resource fallback hosting — use database driver
CACHE_STORE=database
SESSION_DRIVER=database
QUEUE_CONNECTION=database

# Production with Redis, when available
CACHE_STORE=redis
...
> **Production note:** Azure VM production may use Redis for cache/queue when Redis is provisioned. Keep sessions in the database unless the Redis instance is persistent and monitored.
...
UEConnect production is deployed to an **Azure VM VPS** via **GitHub Actions + Docker**. The `CI` workflow runs tests and a Docker build on pull requests and pushes to `main`; after `CI` succeeds on `main`, `Deploy Azure VM` connects to the server over SSH and deploys the exact commit SHA.
...
3. Import Career Pathway data when empty
4. `php artisan optimize:clear`
5. Fix storage permissions
6. Supervisord -> manages Nginx, PHP-FPM, Queue Worker, Reverb

Read these docs before changing deployment:
- [`docs/10-devops/azure-vm-setup.md`](docs/10-devops/azure-vm-setup.md) - first-time Azure VM setup.
- [`docs/10-devops/deployment.md`](docs/10-devops/deployment.md) - production deploy runbook.
- [`docs/10-devops/ci-cd.md`](docs/10-devops/ci-cd.md) - GitHub Actions pipeline.
- [`docs/05-system-architecture/deployment-architecture.md`](docs/05-system-architecture/deployment-architecture.md) - deployment topology.
