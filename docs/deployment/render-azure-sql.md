...
## BƯỚC 4: CÁC LỆNH CẦN CHẠY SAU KHI DEPLOY XONG

Khi Render thông báo build thành công và dịch vụ ở trạng thái **Live**:
1. Truy cập dashboard của Render Web Service.
2. Tìm nút **Shell** (nếu có trên gói trả phí) hoặc sử dụng tính năng thực thi lệnh thủ công. Do gói Free của Render hạn chế Shell, bạn có thể thực hiện chạy migration thông qua các route tạm thời hoặc trigger deploy command. Nếu có Shell/Terminal:
   - Dọn dẹp cache cấu hình cũ:
     ```bash
     php artisan optimize:clear
     ```
   - Chạy các tệp Migration để dựng bảng trên Azure SQL (bắt buộc thêm `--force` vì môi trường là production):
     ```bash
     php artisan migrate --force
     ```
   - Tạo dữ liệu mẫu (Seed) nếu cần thiết:
     ```bash
     php artisan db:seed --force
     ```
   - Tạo lại cache cấu hình cho tối ưu tốc độ:
     ```bash
     php artisan config:cache
     ```
     *(Lưu ý: Không chạy `route:cache` nếu ứng dụng có chứa Closure routes vì Laravel sẽ báo lỗi).*
